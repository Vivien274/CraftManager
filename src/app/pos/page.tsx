...
function POSContent() {
  ...
  const handleLoadOrderIntoPOS = (order: any) => {
    clearCart();
  ...
  const handleCheckout = async () => {
    ...
    if (sale) {
      if (linkedOrder) {
        ...
        updateOrderStatus(linkedOrder.id, 'processing');
      }
      setLastCompletedSale({ ...sale, changeToReturn, givenAmount, linkedOrder });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-7 space-y-4">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1">
            ...
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-xs font-bold transition border shrink-0 flex items-center gap-1.5 ${
          ...
          {/* Conditional View: Pending Orders Grid OR Touch Product Cards Grid */}
          {selectedCategory === 'orders' ? (
            <div className="space-y-4 text-left">
              {pendingOrders.length === 0 ? (
                <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-2">
                  ...
                  <h3 className="font-bold text-slate-800 text-sm">Aucune commande en attente de paiement</h3>
                  <p className="text-xs text-slate-500">Toutes les commandes du carnet ont été réglées.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {pendingOrders.map((order) => {
                    const client = clients.find((c) => c.id === order.client_id) || order.client;

                    return (
                      <div
                        key={order.id}
                        className="bg-white rounded-2xl p-5 border border-slate-200 hover:border-amber-400 shadow-sm transition space-y-3 flex flex-col justify-between"
                      >
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <span className="px-2.5 py-0.5 rounded text-[10px] font-black uppercase bg-amber-100 text-amber-900 border border-amber-300">
                              {order.order_number}
                            </span>
                          ...
                          <div>
                            <h4 className="font-black text-slate-900 text-sm">{client?.name || 'Client Inconnu'}</h4>
                            {client?.company_name && (
                              <p className="text-xs text-slate-500 font-medium">{client.company_name}</p>
                            )}
                          </div>
                            ...
                            {order.items.map((item, idx) => {
                              ...
                              return (
                                <div key={idx} className="flex justify-between text-slate-700 text-[11px]">
                                  <span>
                                    {item.quantity}x {prod?.name || 'Produit'}
                                  </span>
                        ...
                        <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                          ...
                          <button
                            type="button"
                            onClick={() => handleLoadOrderIntoPOS(order)}
        ...
        <div className="lg:col-span-5 xl:col-span-5 bg-white rounded-3xl p-6 border border-slate-200 shadow-lg space-y-5 sticky top-20">
          {/* Cart Header: Clean Non-wrapping Header */}
          <div className="flex items-center justify-between pb-3 border-b border-slate-200">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-amber-600 shrink-0" />
              <h2 className="font-black text-slate-900 text-base whitespace-nowrap">Panier en Caisse</h2>
            ...
            {cart.length > 0 && (
              <button
                ...
                className="text-xs text-slate-500 hover:text-red-600 font-bold transition flex items-center gap-1 whitespace-nowrap cursor-pointer"
              >
          ...
          {/* Linked Order Banner */}
          {linkedOrder && (
            <div className="bg-amber-100 text-amber-950 p-2.5 rounded-xl border border-amber-300 text-xs font-bold flex items-center justify-between shadow-xs">
              <div className="flex items-center gap-2">
                <span>📋 Commande {linkedOrder.order_number}</span>
                <span className="text-[10px] bg-amber-200 px-2 py-0.5 rounded-full font-black">
                  {linkedOrder.client?.name || 'Client'}
                </span>
              </div>
              <button
                onClick={() => setLinkedOrder(null)}
                className="text-amber-800 hover:text-amber-950 text-xs underline font-semibold cursor-pointer"
              >
                Détacher
              </button>
            </div>
          ...
          <div className="space-y-2.5 max-h-72 overflow-y-auto pr-1">
            {cart.length === 0 ? (
              ...
              cart.map((item) => (
                <div
                  key={item.product.id}
                  className="bg-slate-50/80 rounded-2xl p-3.5 border border-slate-200 space-y-2 hover:border-slate-300 transition"
                >
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      ...
                      className="text-slate-400 hover:text-rose-600 transition p-1 rounded-lg hover:bg-slate-200 shrink-0 cursor-pointer"
                  ...
                  <div className="flex items-center justify-between text-xs pt-1.5 border-t border-slate-200/70">
                    ...
                    {/* Stepper with larger comfortable buttons */}
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => updateCartQuantity(item.product.id, item.quantity - 1)}
                        className="w-7 h-7 rounded-lg bg-white border border-slate-300 text-slate-800 flex items-center justify-center hover:bg-slate-100 font-black shadow-2xs cursor-pointer transition active:scale-95"
                      >
                      ...
                      <button
                        onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)}
                        className="w-7 h-7 rounded-lg bg-indigo-600 text-white flex items-center justify-center hover:bg-indigo-500 font-black shadow-2xs cursor-pointer transition active:scale-95"
                      >
      ...
      {isReceiptModalOpen && lastCompletedSale && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl p-6 border border-emerald-300 text-center space-y-4 shadow-2xl">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900">Vente Encaissée !</h2>
              <p className="text-xs text-slate-500 mt-1">Stock savon déstocké automatiquement avec succès.</p>
              {lastCompletedSale.linkedOrder && (
                <div className="mt-2 p-2 bg-emerald-50 text-emerald-900 border border-emerald-300 rounded-xl text-xs font-bold">
                  📋 Commande N° {lastCompletedSale.linkedOrder.order_number} marquée comme PAYÉE !
                </div>
              )}
            </div>
